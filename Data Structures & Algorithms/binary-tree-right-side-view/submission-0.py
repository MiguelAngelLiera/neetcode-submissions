...
class Solution:
    ...
    #                 q.append(last)
    def rightSideView(self, root: Optional[TreeNode]) -> List[int]:
        res = self.aux_rightSideView(root)
        return res

    def aux_rightSideView(self, root: Optional[TreeNode]) -> List[int]:
        if root is None:
            return []
        
        rights = [root.val]

        side_view_right = self.aux_rightSideView(root.right)
        side_view_left = self.aux_rightSideView(root.left) 
